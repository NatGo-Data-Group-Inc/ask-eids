...
export async function loadDentalLifecycleArtifacts() {
  const rows = await parseManifest();
  const dentalRows = rows.filter((row) => row.product_id === 'dental' && row.wave !== 'wave-00-baseline');
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'askeids-lifecycle-'));
...
export async function uploadLifecycleArtifact(page, descriptor) {
  ...
  if (!uploadResponse.ok()) {
    throw new Error(`Artifact upload request failed for "${descriptor.title}" with status ${uploadResponse.status()}.`);
  }
  ...
  try {
    ...
    throw new Error(`Upload modal did not close for "${descriptor.title}". ${visibleErrors.join(' | ') || 'No scoped validation message was available.'}`);
  }
